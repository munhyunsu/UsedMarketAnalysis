...
def parse_html(file_path):
    ...
    # 게시글 가격을 가져오는 부분.
    # tables = soup.find_all("table", { "id" : "marketread" }) # 시간과 동일
    article_prize = tables[0].find_all("td", {"style": "background-color: #ffffff;" })[4].contents[0].encode('utf-8')[:-5]

    # 게시글 전화 번호를 가져오는 부분.
    #article_phone = str(tables[0].find_all("td", { "background-color" : "#ffffff" })[3]).find("####")
    article_phone = str(tables[0].find_all("td", {"style": "background-color: #ffffff;" })[3]).find("####")
    if article_phone == -1:
        article_phone = False
    else:
        article_phone = True
