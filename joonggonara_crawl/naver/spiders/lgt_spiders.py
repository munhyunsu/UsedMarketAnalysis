...
class LgtSpider(scrapy.Spider):
	...
	target_file = open('target_lgt.txt', 'r')
	crawl_target = int(target_file.readline())
	target_file.close()
	max_file = open('max_lgt.txt', 'r')
	...
	def after_login(self, response):
		# 로그인 디버깅용.
		filename = 'output/login_lgt.html'
		with open(filename, 'wb') as f:
			f.write(response.body)
		f.close()
		# 혹시 로그인이 실패했을 경우.
		...
		if 'authentication failed' in response.body:
			...
			# 다운로드 리스트 초기화
			list_f = open('output/list_lgt.txt', 'w')
			list_f.close()
			# 로그인 성공 후 게시판에서 각 게시글의 URL을 따옴.
			# 이땐 공지사항 URL도 같이 들어옴.
			# SKT - 339, KT - 424, LGT - 425
			# 여성상의(fup) - 356, 여성하의(fdown) - 357, 남성상의(mup) - 358, 남성하의(mdown) - 359
			return Request(url='http://cafe.naver.com/ArticleList.nhn?search.boardtype=L&userDisplay=50&search.menuid=425&search.questionTab=A&search.clubid=10050146&search.specialmenutype=&search.totalCount=501&search.page=1', callback=self.parse_list)
	...
	def parse_list(self, response):
		...
		list_f = open('output/list_lgt.txt', 'ab')
		...
		for i in response.xpath('//a[contains(@href, "articleid") and not(contains(@href, "specialmenutype"))]/@href').extract():
			if crawl_count >= crawl_target:
				...
				if os.path.isfile('output/lgt/' + article_num + '.html'):
		...
		if ((crawl_count >= crawl_target) or (page_num >= max_page)):
			...
			next_url = 'http://cafe.naver.com/ArticleList.nhn?search.boardtype=L&userDisplay=50&search.menuid=425&search.questionTab=A&search.clubid=10050146&search.specialmenutype=&search.totalCount=501&search.page=' + str(page_num+1)
			return Request(url=next_url, callback=self.parse_list)
		
	# 게시글 수집.
	def crawl_article(self):
		read_list_f = open('output/list_lgt.txt', 'r')
	...
	def parse_article(self, response):
		...
		filename = 'output/lgt/' + re.split(r'[?=&]', response.url)[12] + '.html'
		with open(filename, 'wb') as f:
			f.write(response.body)
		f.close()
