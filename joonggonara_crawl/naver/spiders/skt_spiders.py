...
class SktSpider(scrapy.Spider):
	...
	def after_login(self, response):
		...
		if 'authentication failed' in response.body:
			...
			return Request(url='http://cafe.naver.com/ArticleList.nhn?search.boardtype=L&userDisplay=50&search.menuid=339&search.questionTab=A&search.clubid=10050146&search.specialmenutype=&search.totalCount=501&search.page=1', callback=self.parse_list)
	...
	def parse_list(self, response):
		...
		if ((crawl_count >= crawl_target) or (page_num >= max_page)):
			...
			next_url = 'http://cafe.naver.com/ArticleList.nhn?search.boardtype=L&userDisplay=50&search.menuid=339&search.questionTab=A&search.clubid=10050146&search.specialmenutype=&search.totalCount=501&search.page=' + str(page_num+1)
	...
	def parse_article(self, response):
		...
		filename = 'output/skt/' + re.split(r'[?=&]', response.url)[12] + '.html'
		with open(filename, 'wb') as f:
			f.write(response.body)
		f.close()
