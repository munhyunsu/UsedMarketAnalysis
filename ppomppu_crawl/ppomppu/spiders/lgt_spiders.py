...
class LgtSpider(scrapy.Spider):
	...
	def after_login(self, response):
		...
		if 'authentication failed' in response.body:
			...
			# 이땐 공지사항 URL도 같이 들어옴.
			# SKT - 2, KT - 3, LGT - 5
			return Request(url='http://www.ppomppu.co.kr/zboard/zboard.php?id=market_phone&page=1&category=5&divpage=196', callback=self.parse_list)
	...
	def parse_list(self, response):
		...
		if ((crawl_count >= crawl_target) or (page_num >= max_page)):
			...
			next_url = 'http://www.ppomppu.co.kr/zboard/zboard.php?id=market_phone&page=' + str(page_num+1) + '&category=5&divpage=196'
	...
	def parse_article(self, response):
		...
		filename = 'output/lgt/' + article_num + '.html'
		with open(filename, 'wb') as f:
			f.write(response.body)
		f.close()
	...
	def parse_phone(self, response, article_num):
		filename = 'output_phone/lgt/' + article_num + '.html'
		with open(filename, 'wb') as f:
			f.write(response.body)
		f.close()
