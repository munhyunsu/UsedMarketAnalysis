...
class OlxSpider(scrapy.Spider):
    """올렉스 스파이더
    올렉스 데이터를 수집하는 스파이더 부모 클래스
    """
    #name = None # 스파이더 이름
    name = 'olx'

    def __init__(self, ini_file = 'olx.ini'):
        """초기화
        """
        if self.name == None:
    ...
    def _prepare_output(self):
        ...
        cursor.executescript('''
                ...
                name + ''' (
                article_url TEXT PRIMARY KEY NOT NULL UNIQUE
                );
    ...
    def parse(self, response):
        ...
        for aurl in response.xpath(article_url_form).re(article_url_re):
            # 수집 대상 목록 확인
            cursor.execute('''
            ...
            if cursor.fetchone() is not None:
                logging.info('skip: {0}'.format(aurl))
                continue
            # 수집 대상에 추가
            cursor.execute('''
                    INSERT OR IGNORE INTO list_''' + name +
                    ''' (article_url) VALUES ("''' + aurl +
                    '''")''')
            connector.commit()
        # 제한 페이지를 넘었는지 확인
        try:
        ...
        if (list_length >= goal) or (pnum >= max_page):
            ...
            request_url = base_url[:-1] + '?o=' + str(pnum + 1)
    ...
    def crawl_article(self):
        ...
        # Request 보내기
        for aurl in article_list:
            # url 조립
            request_url = aurl[0]
            # Request를 날리기 전 목록에서 제거
            cursor.execute('''
                    DELETE FROM list_''' + name +
                    ''' WHERE article_url = "''' + request_url + '"')
            connector.commit()
            # 랜덤 sleep
