...
class PspSpider(scrapy.Spider):
	...
	target_file = open('target_psp.txt', 'r')
	crawl_target = int(target_file.readline())
	target_file.close()
	max_file = open('max_psp.txt', 'r')
	...
	def after_login(self, response):
		# 로그인 디버깅 용.
		filename = 'output/login_psp.html'
		...
		if 'authentication failed' in response.body:
			...
			# 다운로드 리스트 초기화
			list_f = open('output/list_psp.txt', 'w')
			list_f.close()
			# 로그인 성공 후 게시판에서 각 게시글의 URL을 따옴.
			# 이땐 공지사항 URL도 같이 들어옴.
			# 3DS = market_nds VITA = market_psp PS4 = market_ps XBOX = market_xbox Wii = market_ngc
			return Request(url='http://market.ruliweb.daum.net/list.htm?table=market_psp&find=&ftext=&ftext2=&ftext3=3&page=1', callback=self.parse_list, dont_filter=True)
	...
	def parse_list(self, response):
		...
		list_f = open('output/list_psp.txt', 'ab')
		# 사용자가 작성한 게시글 파악
		for i in response.xpath('//a[contains(@href, "read.htm?table=market_psp")]/@href').extract():
			if crawl_count >= crawl_target:
				break
			else:
				#print i
				article_num = re.split(r'[?=&]', i)[6]
				if os.path.isfile('output/psp/' + article_num + '.html'):
					print 'target skip: ' + article_num
				else:
					article_url = 'http://market.ruliweb.daum.net/read.htm?table=market_psp&page=&num=' + article_num + '\n'
		...
		if ((crawl_count >= crawl_target) or (page_num >= max_page)):
			...
			next_url = 'http://market.ruliweb.daum.net/list.htm?table=market_psp&find=&ftext=&ftext2=&ftext3=3&page=' + str(page_num+1)
			return Request(url=next_url, callback=self.parse_list, dont_filter=True)
		
	# 게시글 수집.
	def crawl_article(self):
		read_list_f = open('output/list_psp.txt', 'r')
	...
	def parse_article(self, response):
		...
		filename = 'output/psp/' + article_num + '.html'
		with open(filename, 'wb') as f:
			f.write(response.body)
		f.close()
