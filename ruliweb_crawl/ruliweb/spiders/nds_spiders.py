...
class NdsSpider(scrapy.Spider):
	...
	def after_login(self, response):
		# 로그인 디버깅 용.
		filename = 'output/login_nds.html'
		...
		if 'authentication failed' in response.body:
			...
			# 다운로드 리스트 초기화
			list_f = open('output/list_nds.txt', 'w')
			list_f.close()
	...
	def parse_list(self, response):
		...
		list_f = open('output/list_nds.txt', 'ab')
		# 사용자가 작성한 게시글 파악
		for i in response.xpath('//a[contains(@href, "read.htm?table=market_nds")]/@href').extract():
			if crawl_count >= crawl_target:
				break
			else:
				#print i
				article_num = re.split(r'[?=&]', i)[6]
				if os.path.isfile('output/nds/' + article_num + '.html'):
					print 'target skip: ' + article_num
				else:
					#article_url = 'http://market.ruliweb.daum.net/read.htm?table=market_nds&page=&num=' + article_num + '&find=&ftext=\n'
					article_url = 'http://market.ruliweb.daum.net/read.htm?table=market_nds&page=&num=' + article_num + '\n'
	...
	def crawl_article(self):
		read_list_f = open('output/list_nds.txt', 'r')
	...
	def parse_article(self, response):
		...
		#filename = 'output/rnds/' + re.split(r'[?=&]', response.url)[6] + '.html'
		filename = 'output/nds/' + article_num + '.html'
		with open(filename, 'wb') as f:
			f.write(response.body)
		f.close()
